...
BEGIN
select * from message where (User_user_id = id or  User_user_id1 = id) and is_buddy_request = 0;

  
END |

DELIMITER |
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_buddy_requests`(in id int)
BEGIN
select * from message where (User_user_id = id or  User_user_id1 = id) and is_buddy_request = 1;
...
DELIMITER |
CREATE DEFINER=`root`@`localhost` PROCEDURE `login`(in us varchar(40), in pass varchar(40), out user_id int)
BEGIN
